...
function renderModalExpirySuggest() {
  const el = $("modalExpirySuggest");
  if (!el) return;
  const list = getStock().filter((item) => (item.qty != null ? item.qty : 1) > 0);
  const items = [];
  ...
  list.forEach((item) => {
    const status = getExpiryStatus(item.expiry);
    if (status === "expired" || status === "soon" || status === "week") {
  ...
  // Sort: expired first, then soon, then week
  const order = { expired: 0, soon: 1, week: 2 };
  items.sort((a, b) => order[a.status] - order[b.status]);

  let html = `<div class="modal-expiry-suggest-title">⏰ 期限が近い食材</div>`;
  html += `<div class="modal-expiry-chips">`;
  items.forEach((item) => {
    html += `<button type="button" class="modal-expiry-chip modal-expiry-chip--${item.status}" data-name="${escapeHtml(item.name)}">${escapeHtml(item.name)} <small>${formatExpiry(item.expiry)}</small></button>`;
  });
  html += `</div>`;
  el.innerHTML = html;
  el.hidden = false;

  // Click: put item name into the first empty entry input as search hint
  el.querySelectorAll(".modal-expiry-chip").forEach((chip) => {
    chip.addEventListener("click", () => {
      const name = chip.dataset.name;
      const emptyIdx = menuEntries.findIndex((e) => !e.value.trim());
      if (emptyIdx !== -1) {
        menuEntries[emptyIdx].value = name;
      } else {
        menuEntries.push({ value: name, mode: "manual" });
      }
      renderMenuEntries();
    });
  });
